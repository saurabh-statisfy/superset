...
export function useKnownHosts(): {
	...
	// An offline build talks to no cloud, so the roster query can only ever
	// fail and no cache is ever written. Without the error clause every
	// downstream `settled` gate stays false for the life of the process.
	const settled =
		hostsQuery.data !== undefined ||
		(env.SKIP_ENV_VALIDATION && hostsQuery.isError);
